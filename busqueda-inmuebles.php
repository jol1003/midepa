<?
  ...
  $usrid=testins($_GET["usrid"]);
  if($usrid!=""){
	if(!is_numeric($usrid)){
	  header("location:".URL_PATH);
	}else{
	  $fil="AND usuario_id='".$usrid."'";
	}
  }
  $bus=testins($_GET["txtbus"]);
  if($bus!=""){
	$ope=testins($_GET["cboope"]); $tip=testins($_GET["cbotip"]); $dor=testins($_GET["cbodor"]); $pmn=testins($_GET["txtpmn"]); $pmx=testins($_GET["txtpmx"]);
	$fil="AND tipo='".$ope."' AND (";
	$bus=explode(",", $bus); foreach($bus as $bu){$fil.="distrito_id=".$bu." OR ";} $fil=substr($fil, 0, strlen($fil)-4).")";
	if($tip!=""){$fil.=" AND inmuebletipo_id=".$tip;}
	if($dor!=""){$fil.=" AND dormitoriosmin>=".$dor;}
	if($pmn!=""){$fil.=" AND preciomin>=".$pmn;}
	if($pmx!=""){$fil.=" AND preciomin<=".$pmx;}
  }
  //PAGINANDO
  $pag=testins($_GET["pag"]);
  if(!$pag){$ini=0; $pag=1;}else{$ini=($pag - 1) * 21;}
  $pager=$db->fetchObject("SELECT COUNT(id) AS can FROM inmueble WHERE estado='1' ".$fil);
  $totreg=$pager->can;
  $totpag=20;
  $total=ceil($totreg / $totpag);
  $prev=$pag - 1; if($prev==0){$prev=$total;}
  $next=$pag + 1; if($next > $total){$next=1;}
  $upag=URL_PATH."busqueda-inmuebles/page/";
  
  $i=0;
  $inms=$db->query("SELECT id,nombre,urlnombre,imagen,antiguedad,direccion,areatotalmin,areatotalmax,areaconstruidamin,areaconstruidamax,preciomin,preciomax,preciodolaresmin,preciodolaresmax,adicionales,fechacreacion,tipopago FROM inmueble WHERE estado='1' AND '".date("Y-m-d")."' BETWEEN fechainicio AND fechafin ".$fil." ORDER BY tipopago DESC,fechacreacion DESC LIMIT ".$ini.",".$totpag);
  while($inm=mysql_fetch_object($inms)){
	...
	if($inm->tipopago!="0"){$ainm[$i]["cla"]=' class="asp"';}
	$i++;
  }
  $caninm=count($ainm);
...
<div class="izq">
  <div class="bkg itp">
      ...
      <div class="bim">
      <form id="frmBus" name="frmBus" method="get" action="<?=URL_PATH;?>busqueda-inmuebles/"><div class="mad"><div class="slef"><select name="cboope" class="txt"><option value="1">Compra</option><option value="2">Venta</option></select></div><div class="flef"><select name="cbotip" class="txt"><option value="">[Todos los tipos]</option><option value="1">Casa</option><option value="2">Departamento</option><option value="3">Habitaci&oacute;n</option><option value="4">Terreno</option></select></div><div class="flot"></div></div><div class="bux"><div class="bus"><input type="text" name="txtbus" id="txtbus" /></div><div class="btn btx"><span><input type="submit" id="btnbus" value="Buscar" /></span></div><div class="flot"></div></div></form>
      </div>
    </div>
  </div>
<?
